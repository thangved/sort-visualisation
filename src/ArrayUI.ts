...
class ArrayUI {
	...
	drawTexts() {
		this.elements.forEach((element, index) => {
			if (element.step < element.key) {
				if (!index) element.step++;
				if (
					index &&
					this.elements[index - 1].key / 32 <=
						this.elements[index - 1].step
				)
					element.step++;
			}
	...
	drawBaseElement(element: ArrayUIElement) {
		...
		this.context.rect(
			...
			element.position.y +
				this.options.dy -
				(element.step - this.getMaxAllElements()) * 5,
			this.options.width,
			5 * element.step
		);
	...
	updateSwap() {
		if (!this.swapping) return this.resetPosition();
	...
	private __bubbleSort(): void {
		...
		if (this.index2 !== -1)
			if (
				this.elements[this.index2]?.key >
				this.elements[this.index2 + 1]?.key
			) {
	...
	private __selectionSort(): void {
		...
		if (this.swapping) {
			this.timeouts.push(
				setTimeout(() => this.__selectionSort(), this.getSortSpeed())
			);
			return;
		}
		...
		if (this.index2 !== this.length - 1) this.index2++;
	...
	private __insertionSort(): void {
		...
		if (this.swapping) {
			this.timeouts.push(
				setTimeout(() => this.__insertionSort(), this.getSortSpeed())
			);
			return;
		}
